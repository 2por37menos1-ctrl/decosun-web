-- DecoSun ERP - Phase 3A.12
-- Surgical project payment permission for Edgar Leighton / Iquique.
-- This keeps Treasury, commissions, costs, reports, and global finance permissions unchanged.
...
begin
  -- If any step fails, the payment event, treasury movement, and project cache
  -- updates roll back. Financial history is never deleted or overwritten here.
  ...
  if not (
    ...
    or (v_is_edgar_iquique and v_is_north_project)
  ) then
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'permission_scope', case
        when v_profile.role = 'gerencia' then 'gerencia'
        when v_is_edgar_iquique then 'edgar_iquique_project_payment'
        else 'unknown'
      end
    )
  )
  ...
  -- 6. Keep amount_paid unchanged. It remains legacy/cache compatibility.
...
comment on function public.register_project_payment(
  ...
  'DecoSun Financial Engine Phase 3A.12: registers real customer payments. Permission is gerencia globally, plus Edgar/Edgar Leighton as jefatura_region Iquique for north/Iquique projects only.';
